...
def drop_header_if_present(df: pd.DataFrame) -> pd.DataFrame:
    # Καθαρίζει το header αν υπάρχει
...
def read_triples(path: str) -> pd.DataFrame:
    # Διαβάζει τριπλέτες. Αγνοεί πιθανά headers.
    try:
        df = pd.read_csv(path, sep="\t", header=None, names=["h","r","t"])
        # Έλεγχος αν η πρώτη γραμμή είναι header
...
def main():
    ...
    # Ενωση όλων για να βρούμε τα μοναδικά
    all_df   = pd.concat([train_df, dev_df, test_df], ignore_index=True).drop_duplicates()

    # Τα πραγματικά entities και relations που χρησιμοποιούνται στο dataset
    ...
    # Φιλτράρουμε: Κρατάμε περιγραφές ΜΟΝΟ για entities που υπάρχουν στα triples
    ent_map = dict(zip(ent["uri"], ent["description"]))
    final_ent_data = []
    for e in entities:
        desc = ent_map.get(e, f"Entity {e.split('/')[-1]}") # Default αν λείπει
    ...
    # Ελέγχουμε ποια περισσεύουν (για ενημέρωση)
