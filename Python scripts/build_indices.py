# build_indexes_efficient.py
from llama_index.core import VectorStoreIndex, Settings
from llama_index.embeddings.huggingface import HuggingFaceEmbedding
import warnings

# Import the data extraction function from your other script
from prepare_data import extract_and_format_data
...
def build_and_persist_indexes():
    """Builds and saves the three specialized indexes without needing a local LLM."""
    
    print("--- Setting up LlamaIndex embedding model ---")
    # For indexing, we ONLY need the embedding model.
    # This makes the script much lighter and faster.
    try:
    ...
    # We explicitly set the LLM to None to avoid any unnecessary loading.
    Settings.llm = None
    print("LLM is not required for indexing. Proceeding with embedding model only.")

    # 1. Get the data
    entity_docs, class_docs, prop_docs = extract_and_format_data()
